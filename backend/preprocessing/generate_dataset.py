...
def process_row(row_dict, num_queries_per_field, is_train=True):
    ...
    # Add fallback for rows with no description
    if not desc_queries:
        for _ in range(num_queries_per_field):
            row_data.append({
                'text': f"books about {row_dict.get('categories', 'books')}",
                'label': 'DESCRIPTION_SEARCH'
            })
    else:
...
def generate_smart_data_multiprocessing(
    df,
    train_output_file="train_search_intent_data_temp.csv",
    test_output_file="test_search_intent_data_temp.csv",
    batch_size=10000,
    num_queries_per_field_train=1,
    num_queries_per_field_test=1,
    num_workers=None
):
    """
